<x-administrador-layout>
    <div class="bg-white shadow-md rounded-lg p-6 mb-6">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-semibold text-gray-800">Semestres - Acta de Calificaciones</h2>
            <a href="{{ route('administrador.semestres.create') }}"
            ...
            <table class="min-w-full table-auto border-collapse">
                <thead>
                    <tr class="bg-gray-800 text-white">
                        ...
                        <th class="px-4 py-2 text-center">Orden</th>
                ...
                <tbody class="bg-white">
                    @forelse ($semestres as $key => $semestre)
                        <tr class="border-b">
                            <td class="px-4 py-2 text-center">{{ $semestre->id }}</td>
                            <td class="px-4 py-2 text-center">{{ $semestre->name }}</td>
                            <td class="px-4 py-2 text-center">{{ $semestre->code }}</td>
                            <td class="px-4 py-2 text-center">{{ $semestre->order }}
                            </td>
                            ...
                            <td class="px-4 py-2 flex items-center justify-center space-x-4">
                                <a href="{{ route('administrador.semestres.edit', $semestre) }}"
                                ...
                                <form action="{{ route('administrador.semestres.destroy', $semestre) }}" method="POST"
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-4 py-2 text-center text-gray-500">
                                No hay ningun semestre registrado.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
