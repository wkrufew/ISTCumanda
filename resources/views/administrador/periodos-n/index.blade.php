<x-administrador-layout>
    <div class="bg-white shadow-md rounded-lg p-6 mb-6">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-semibold text-gray-800">Periodos - Acta de Calificaciones</h2>
            <a href="{{ route('administrador.periodos.create') }}"
            ...
            <table class="min-w-full table-auto border-collapse">
                ...
                <tbody class="bg-white">
                    @forelse ($periodos as $key => $periodo)
                        <tr class="border-b">
                            <td class="px-4 py-2">{{ $periodo->id }}</td>
                            <td class="px-4 py-2">{{ $periodo->name }}</td>
                            <td class="px-4 py-2">{{ $periodo->code }}</td>
                            <td class="px-4 py-2">{{ $periodo->start_date->isoFormat('D [de] MMM [de] Y') }}
                                <br>{{ $periodo->end_date->isoFormat('D [de] MMM [de] Y') }}
                            </td>
                            ...
                            <td class="px-4 py-2 flex items-center justify-center space-x-4">
                                <a href="{{ route('administrador.periodos.edit', $periodo) }}"
                                ...
                                <form action="{{ route('administrador.periodos.destroy', $periodo) }}" method="POST"
